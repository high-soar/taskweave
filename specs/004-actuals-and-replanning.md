...
- **背景と目的**:
  ...
  本仕様では、原本ファイル（`actuals.yaml`, `calendar.yaml`）の検証規則（Issue #25）に加え、起算日（As-of Date）を用いた過去実績の固定と未完了タスクの未来スケジュール最適化エンジン（Issue #26）のアルゴリズムと制約を定義します。
...
3. **未来の個別不在（Absences）による日別キャパシティ自動低減**:
   - 本仕様は起算日による実績固定と残工数再計画（Issue #26）を対象とします。未来のメンバ不在（`calendar.absences`）を考慮したソルバー割当制御は次課題 Issue #27 で実装します。
4. **ベースライン計画との差分（Diff）算出と遅延原因診断**:
