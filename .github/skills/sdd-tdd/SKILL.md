...
3. **Address Review Feedback**:
   ...
   - Fix all `[MUST]` items in code and tests (`[MUST]` cannot be deferred).
   - Fix accepted `[SHOULD]` items. For deferred non-blocking items, **always file a dedicated GitHub Issue** to track it in the backlog, and link the issue number in the PR reply comment.
   - For `[QUESTION]` items or non-adopted `[IMO]` suggestions, provide clear technical rationale or YAGNI explanation in the reply.
   - Commit, push, and reply to each `[Rx]` in the PR comments following the standard response format (policy: fix/accept/defer/won't fix, reason, commit, test result, remaining risk).
4. **Invoke Re-review & Verify Resolution**:
   - Invoke `pr-reviewer` again to perform a re-review of the updated diff.
   - Verify that all review items are marked as `RESOLVED` (fixed, properly tracked via an open GitHub Issue, answered, or agreed won't-fix). If any `UNRESOLVED` items remain, repeat the fix/issue cycle.
