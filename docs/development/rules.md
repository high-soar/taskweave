...
- **回答も PR コメントで行う**: PR 作成者または修正担当者は、各指摘の番号（`[R1]` 等）を参照して、対応方針（修正、受け入れ、見送り、対応不要）、理由、変更コミット、検証結果、残るリスクを同じ PR のコメントまたはレビュー・スレッドへ記載します。ローカル修正やコミットメッセージだけでは回答済みとしません。
- **見送り事項の Issue 化義務**: レビューで非ブロッカー項目（`[SHOULD]` 等）について「見送り（将来対応・リファクタリング）」と判断した指摘事項は、PR コメントの記録にとどまらず、必ず独立した GitHub Issue を起票してバックログへ登録し、PR コメントに対応 Issue 番号をリンクします（※`[MUST]` 指摘の見送りは不可）。PR マージ後に技術的負債や改善課題が埋もれ、見直す機会が失われることを防ぎます。
- **再レビューを記録する**: 修正後はレビュアー（または `pr-reviewer`）が同じ PR の更新された差分と検証結果を確認し、解決した指摘（`RESOLVED`）と未解決の指摘（`UNRESOLVED`）をコメントで明示します。指摘を解決済みとするのは、修正と検証、または妥当な見送り理由と追跡 Issue 起票（非ブロッカーのみ）、あるいは合意による対応不要を確認した後に限ります。

### 3.4 レビュー完了条件

- 承認またはマージを依頼する前に、すべての指摘が解決（`[MUST]` はコード・テスト修正完了、非ブロッカーの `[SHOULD]` 等は修正完了または追跡 Issue 起票済みの見送り、`[IMO]` 等は不採用合意、`[QUESTION]` は回答完了）され、再レビューにて全件 `RESOLVED` と判定され、未解決リスクと対応予定が PR コメントに明示され、必要なテストと CI の結果が記録されていることを確認します。
...
- **PR の承認とマージは人間が行う**: Pull Request の最終承認およびマージの権限は人間（開発者・レビュアー）にあります。エージェントは PR 作成、CI 正常終了確認、独立サブエージェント（`pr-reviewer`）によるプレレビューおよび再レビュー完了確認までを担当し、ユーザーから明示的な指示がない限り、自律的に PR をマージしてはなりません。
...
プルリクエストおよび `main` への push では、上記と同じ品質チェックが GitHub Actions CI により網羅的に自動実行されます。開発およびレビュー指摘対応時の迅速なイテレーションを維持するため、push ごとのローカル強制実行（Git フック）は行わず、手動実行または CI での自動検証に集約しています。
